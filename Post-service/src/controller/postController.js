...
const invalidCache = async (req, input) => {
  const cachedKey = `post:${input}`; //single post key
  await req.redisClient.del(cachedKey); //delete single cached post

  const keys = await req.redisClient.keys("posts:*"); //extract all keys start with posts
  if (keys.length > 0) {
...
const createPost = async (req, res) => {
  ...
  try {
    ...
    await invalidCache(req,newlyCreatedPost._id.toString());
...
const getAllPosts = async (req, res) => {
  try {
    ...
    await req.redisClient.setex(cacheKey, 300, JSON.stringify(results)); //set with expiration
...
const deletePost = async (req, res) => {
  try {
    ...
    await invalidCache(req,post._id.toString());
    res.status(200).json({
